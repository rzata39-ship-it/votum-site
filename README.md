...
| Case studies (`/case-studies/<slug>`): slugs, technologies, related services / cases | `src/config/cases.js`; copy in `cases.items` in `src/i18n/translations.js` |
| Solution pages (`/solutions/<slug>`): technologies, linked cases / services, inbound links | `src/config/solutions.js`; copy in `solutionPages` in `src/i18n/translations.js` |
...
- **Case studies** — `/case-studies` (hub) and `/case-studies/<slug>` come from `CASES` in `src/config/cases.js`: route, prerendered HTML, sitemap entry, homepage card link, hub card, service-page "Relevant project experience" (a service lists every case whose `services` contain it), related cases, the "Published case studies" count and breadcrumb `Home > Case Studies > …`. Copy: `cases.items[slug]` in `translations.js`. **All five projects predate the company**: every case page shows the attribution note (`casePage.note`); never write "VOTUM delivered…" for them. The team's history (`company.teamSince`, 2018) is the team's, not the company's (`foundingYear`, 2024). To add a case: add it to `CASES`, add `cases.items.<slug>` and `seo.<key>`, and add its `/case-studies/<slug>/index.html` redirect to `vercel.json`.
- **Solution pages** — `/solutions/opentext-alm` comes from `SOLUTIONS` in `src/config/solutions.js`: route, prerendered HTML, sitemap entry, footer link, breadcrumb `Home > OpenText ALM` (no `/solutions` hub while there is one page) and the contextual "Related solution" links on the service / case pages listed in `linkedFrom`. Copy: `solutionPages[slug]` in `translations.js`. Evidence-based only — every statement maps to a public case study, a service page or a confirmed capability; no partner status, certifications, versions or counts.
